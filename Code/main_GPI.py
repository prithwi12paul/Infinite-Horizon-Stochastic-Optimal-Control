...
if __name__ == '__main__':
    ...
    gpi_solver = GPI(traj,obstacles)  ## shape T X 2

    # err_state_space = error_state  ## shape 4 x T
    ...
    control_space = np.load('control_space.npy')
    error_state_space = np.load('error_state_space.npy')
    avg_error = []

    curr_err_state = np.zeros(4)
    while (cur_iter * time_step < sim_time):
        ...
        curr_error = cur_state - cur_ref
        curr_error[2] = (curr_error[2] + np.pi) % (2*np.pi) - np.pi
        curr_state = np.array([cur_iter,curr_error[0],curr_error[1],curr_error[2]])

        diff = np.linalg.norm(error_state_space.T - curr_state, axis = 1)

        curr_error_idx = np.argmin(diff) 

        control_idx = optimal_policy[curr_error_idx]

        control_action = control_space[:,control_idx]
        print("[v,w]", control_action)

      
        ################################################################

        # Apply control input
        next_state = car_next_state(time_step, cur_state, control_action, noise=True)
        ...
        avg_error.append(np.linalg.norm(cur_state - cur_ref))
        cur_iter = cur_iter + 1
    ...
    print('Average error: ', error/cur_iter)
